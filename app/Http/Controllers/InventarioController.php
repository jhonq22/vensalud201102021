...
public function import() 
    ...
    public function ReporteInventarioGeneral($fecha1, $fecha2, $fecha3, $fecha4)
    {
        ...
        ORDER BY inventarios.id DESC", [$fecha1, $fecha2, $fecha3, $fecha4]);
    ...
    public function ReporteInventarioEstadal($estado_id, $fecha1, $fecha2, $fecha3, $fecha4 )
    {
        $inventarios = DB::select("SELECT historico_estatus.inventario_id, centro_salud, estado, servicio_medico, equipo, marca, serial, numero_bien_nacional, reparado, estatu_equipo, historico_estatus.observacion, historico_estatus.created_at
        FROM historico_estatus
        LEFT JOIN inventarios ON historico_estatus.inventario_id = inventarios.id
        ...
        LEFT JOIN equipos ON inventarios.equipo_id = equipos.id
        LEFT JOIN marcas ON inventarios.marca_id = marcas.id
            WHERE inventarios.estado_id = ?  AND (DATE(historico_estatus.created_at) BETWEEN  ? AND ? OR
         DATE(historico_estatus.updated_at) BETWEEN  ? AND ?)
            ORDER BY inventarios.id DESC", [$estado_id, $fecha1, $fecha2, $fecha3, $fecha4] );

        echo json_encode($inventarios); // para pasar en json
    }


    public function ReporteInventarioMunicipal($municipio_id, $fecha1, $fecha2, $fecha3, $fecha4 )
    {
        $inventarios = DB::select("SELECT historico_estatus.inventario_id, centro_salud, estado, servicio_medico, equipo, marca, serial, numero_bien_nacional, reparado, estatu_equipo, historico_estatus.observacion, historico_estatus.created_at
        FROM historico_estatus
        LEFT JOIN inventarios ON historico_estatus.inventario_id = inventarios.id
        ...
        LEFT JOIN equipos ON inventarios.equipo_id = equipos.id
        LEFT JOIN marcas ON inventarios.marca_id = marcas.id
            WHERE inventarios.municipio_id = ? AND (DATE(historico_estatus.created_at) BETWEEN  ? AND ? OR
         DATE(historico_estatus.updated_at) BETWEEN  ? AND ?)
            ORDER BY inventarios.id DESC", [$municipio_id, $fecha1, $fecha2, $fecha3, $fecha4] );

        echo json_encode($inventarios); // para pasar en json
    }

    public function ReporteInventarioCentroSalud($centro_salud_id, $fecha1, $fecha2, $fecha3, $fecha4)
    {
        $inventarios = DB::select("SELECT  historico_estatus.inventario_id, centro_salud, estado, servicio_medico, equipo, marca, serial, numero_bien_nacional, reparado, estatu_equipo, historico_estatus.observacion, historico_estatus.created_at
        FROM historico_estatus
        LEFT JOIN inventarios ON historico_estatus.inventario_id = inventarios.id
        ...
        LEFT JOIN equipos ON inventarios.equipo_id = equipos.id
        LEFT JOIN marcas ON inventarios.marca_id = marcas.id
            WHERE inventarios.centro_salud_id = ? AND (DATE(historico_estatus.created_at) BETWEEN  ? AND ? OR
         DATE(historico_estatus.updated_at) BETWEEN  ? AND ?)
            ORDER BY inventarios.id DESC", [$centro_salud_id, $fecha1, $fecha2, $fecha3, $fecha4] );
